...
class YouTubePlayerService {
  ...
  loadAndPlay(song, startTime = 0) {
    ...
    state.upsertExternalSong?.({ ...song, source: "youtube", youtubeId: videoId });
  ...
  async play(song, startTime = 0) {
    ...
    state.upsertExternalSong?.({ ...song, source: "youtube", youtubeId: videoId });
    state.currentSong = state.getSongById(song.id) || song;
    state.duration = Number(song.duration) || state.duration || 180;
  ...
  isPlaying() {
    ...
    try { return playing !== undefined && this.player?.getPlayerState?.() === playing; } catch (e) { return false; }
  }

  getCurrentTime() {
    try { return Number(this.player?.getCurrentTime?.()) || state.currentTime || 0; } catch (e) { return state.currentTime || 0; }
  }

  getDuration() {
    try { return Number(this.player?.getDuration?.()) || state.duration || 0; } catch (e) { return state.duration || 0; }
  }
