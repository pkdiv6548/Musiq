...
export class AudioEngine {
  ...
  play() {
    ...
    if (isYouTube) {
      ...
      const result = youtubePlayer.play(song, state.currentTime);
  ...
  getFrequencyData() {
    const song = state.currentSong;
    const isYouTube = song && (song.source === "youtube" || song.id?.startsWith("youtube:"));
    if (isYouTube) return this.getYouTubeVisualizerData(false);
  ...
  getWaveformData() {
    const song = state.currentSong;
    const isYouTube = song && (song.source === "youtube" || song.id?.startsWith("youtube:"));
    if (isYouTube) return this.getYouTubeVisualizerData(true);
    if (!this.analyser) return new Uint8Array(64);
    const data = new Uint8Array(this.analyser.frequencyBinCount);
  ...
  // YouTube iframe audio is not exposed to this page's Web Audio graph.
  // Generate a deterministic playback-reactive visual signal instead of
  // falsely claiming access to the cross-origin audio samples.
  getYouTubeVisualizerData(waveform = false) {
    const length = waveform ? 128 : 128;
    const data = new Uint8Array(length);
    const t = state.currentTime || 0;
    const playing = state.isPlaying;
    let seed = 0;
    for (const ch of String(state.currentSong?.youtubeId || state.currentSong?.id || "youtube").slice(0, 12)) seed = (seed * 31 + ch.charCodeAt(0)) % 997;
    for (let i = 0; i < length; i++) {
      const phase = t * (0.8 + (seed % 7) * 0.08) + i * 0.19 + seed * 0.01;
      const envelope = playing ? 1 : 0.25;
      if (waveform) {
        data[i] = Math.max(0, Math.min(255, 128 + Math.sin(phase) * 82 * envelope));
      } else {
        const low = (Math.sin(phase * 0.55) + 1) * 0.5;
        const mid = (Math.sin(phase * 1.7 + 1.2) + 1) * 0.5;
        const pulse = Math.max(0, Math.sin(t * 5 + i * 0.11));
        data[i] = Math.max(0, Math.min(255, (low * 90 + mid * 75 + pulse * 70) * envelope + 8));
      }
    }
    return data;
  }
