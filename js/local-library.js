...
export class LocalLibraryService {
  ...
  async init() {
    ...
    this.localTracks = saved.map((t) => ({ ...t, source: "local", isCustom: true }));

    // Restore the actual audio Blob from IndexedDB after a page reload.
    await Promise.all(this.localTracks.map(async (track) => {
      const blob = await storage.getAudioBlob(track.id);
      if (blob) {
        track.blobUrl = this.createTrackUrl(blob);
        const stateTrack = state.getSongById(track.id);
        if (stateTrack) Object.assign(stateTrack, track);
        else state.addCustomSong(track);
        if (Array.isArray(state.queue)) {
          state.queue.forEach((queued, index) => {
            if (queued?.id === track.id) state.queue[index] = track;
          });
        }
      }
    }));
    state.notify("queueChanged", { queue: state.queue, index: state.queueIndex });
    state.notify("localLibraryReady", this.localTracks);
  ...
  async processFiles(fileList) {
    ...
    for (let i = 0; i < fileList.length; i++) {
      ...
      const duplicate = this.localTracks.some((t) =>
        t.fileName === file.name &&
        Number(t.fileSize) === Number(file.size) &&
        (t.lastModified || 0) === (file.lastModified || 0)
      );
      ...
      let artwork = id3?.artworkUrl || "/icon.svg";
      ...
      if (lowerTitle.includes("intezaar") || lowerTitle.includes("intezar") || (lowerTitle.includes("aayiye") && lowerTitle.includes("aapka"))) {
        ...
        artwork = id3?.artworkUrl || "/icon.svg";
