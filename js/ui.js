...
import { ICONS, formatTime, renderMusicCard, renderSongRow, showToast, openModal, closeModal } from "./components.js";
...
export class UIManager {
  ...
  renderRightPanelQueue() {
    ...
    let itemsHtml = state.queue.map((song, idx) => `
      <div class="queue-item ${idx === state.queueIndex ? 'active' : ''}" data-index="${idx}" style="display:flex;align-items:center;gap:10px;padding:8px 12px;border-radius:8px;cursor:pointer;background:${idx === state.queueIndex ? 'rgba(255,45,85,0.12)' : 'transparent'};margin-bottom:4px;">
        ...
        <img src="${song.artwork}" style="width:36px;height:36px;border-radius:6px;object-fit:cover;">
  ...
  renderRightPanelInfo() {
    ...
    this.rightPanelContent.innerHTML = `
      <div style="padding:16px;display:flex;flex-direction:column;gap:16px;">
        <img src="${s.artwork}" style="width:100%;aspect-ratio:1/1;border-radius:12px;object-fit:cover;box-shadow:var(--shadow-md);">
        <div>
      ...
      <div class="hero-banner">
        <div class="hero-content">
          ...
          <div class="hero-actions-row">
            ...
            <button class="hero-audit-chip" id="run-audit-btn" title="View 250+ Features System Audit">
              ${ICONS.check} 250+ Feature Audit
            </button>
          </div>
        </div>
      </div>
  ...
  renderSearch() {
    this.viewport.innerHTML = `
      <div style="max-width:900px;margin:0 auto;">
        ...
        <div style="display:flex;align-items:center;gap:8px;margin-bottom:20px;overflow-x:auto;padding-bottom:4px;" id="search-filter-tabs">
          <button class="search-filter-tab active" data-tab="all" style="padding:6px 16px;border-radius:var(--radius-full);background:var(--color-accent);color:#fff;border:1px solid transparent;font-size:0.82rem;font-weight:600;cursor:pointer;">All Results</button>
          <button class="search-filter-tab" data-tab="youtube" style="padding:6px 16px;border-radius:var(--radius-full);background:var(--color-bg-card);color:var(--color-text-primary);border:1px solid var(--color-border);font-size:0.82rem;font-weight:600;cursor:pointer;display:inline-flex;align-items:center;gap:6px;">
          ...
          <button class="search-filter-tab" data-tab="local" style="padding:6px 16px;border-radius:var(--radius-full);background:var(--color-bg-card);color:var(--color-text-primary);border:1px solid var(--color-border);font-size:0.82rem;font-weight:600;cursor:pointer;">Local Device</button>
          <button class="search-filter-tab" data-tab="artists" style="padding:6px 16px;border-radius:var(--radius-full);background:var(--color-bg-card);color:var(--color-text-primary);border:1px solid var(--color-border);font-size:0.82rem;font-weight:600;cursor:pointer;">Artists & Albums</button>
        </div>
  ...
  renderLocalMusic() {
        ...
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:20px;flex-wrap:wrap;gap:14px;">
          <div>
            <div style="display:flex;align-items:center;gap:10px;">
              <h1 style="font-size:2rem;font-weight:800;">Device Local Music</h1>
              <span class="badge-pill" style="color:var(--color-accent);background:rgba(255,45,85,0.15);font-size:0.75rem;">100% PRIVATE & OFFLINE</span>
            </div>
            <p style="color:var(--color-text-muted);font-size:0.85rem;margin-top:4px;">
              ${localLibraryService.localTracks.length} tracks stored in device browser sandbox
            </p>
          </div>
          <div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap;">
            ${localLibraryService.localTracks.length > 0 ? `
              <button class="btn-primary" id="local-play-all-btn">${ICONS.play} Play All</button>
            ` : ""}
            <button class="btn-secondary" id="local-add-files-btn">
            ...
            <button class="btn-secondary" id="local-add-folder-btn">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M22 19a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h5l2 3h9a2 2 0 0 1 2 2z"></path></svg>
              Add Folder
            </button>
            <input type="file" id="local-file-input" multiple accept="audio/*" style="display:none;">
            <input type="file" id="local-folder-input" webkitdirectory directory multiple style="display:none;">
          </div>
        </div>

        <!-- Sub-tabs navigation -->
        <div style="display:flex;align-items:center;gap:8px;margin-bottom:18px;overflow-x:auto;padding-bottom:4px;" id="local-subtabs">
          <button class="local-tab-btn active" data-subtab="all" style="padding:6px 14px;border-radius:var(--radius-full);background:var(--color-accent);color:#fff;border:none;font-size:0.82rem;font-weight:600;cursor:pointer;">All Songs</button>
          <button class="local-tab-btn" data-subtab="recent-added" style="padding:6px 14px;border-radius:var(--radius-full);background:var(--color-bg-card);color:var(--color-text-primary);border:1px solid var(--color-border);font-size:0.82rem;font-weight:600;cursor:pointer;">Recently Added</button>
          <button class="local-tab-btn" data-subtab="recent-played" style="padding:6px 14px;border-radius:var(--radius-full);background:var(--color-bg-card);color:var(--color-text-primary);border:1px solid var(--color-border);font-size:0.82rem;font-weight:600;cursor:pointer;">Recently Played</button>
          <button class="local-tab-btn" data-subtab="artists" style="padding:6px 14px;border-radius:var(--radius-full);background:var(--color-bg-card);color:var(--color-text-primary);border:1px solid var(--color-border);font-size:0.82rem;font-weight:600;cursor:pointer;">Artists</button>
          <button class="local-tab-btn" data-subtab="albums" style="padding:6px 14px;border-radius:var(--radius-full);background:var(--color-bg-card);color:var(--color-text-primary);border:1px solid var(--color-border);font-size:0.82rem;font-weight:600;cursor:pointer;">Albums</button>
          <button class="local-tab-btn" data-subtab="genres" style="padding:6px 14px;border-radius:var(--radius-full);background:var(--color-bg-card);color:var(--color-text-primary);border:1px solid var(--color-border);font-size:0.82rem;font-weight:600;cursor:pointer;">Genres</button>
        </div>

        <!-- Filter bar -->
        ${localLibraryService.localTracks.length > 0 ? `
          <div style="display:flex;align-items:center;gap:12px;margin-bottom:18px;flex-wrap:wrap;">
            <input type="text" id="local-filter-input" placeholder="Filter local tracks..." style="padding:7px 16px;border-radius:var(--radius-full);background:var(--color-bg-surface-elevated);border:1px solid var(--color-border);font-size:0.85rem;color:var(--color-text-primary);width:240px;">
            <select id="local-sort-select" style="padding:7px 16px;border-radius:var(--radius-full);background:var(--color-bg-surface-elevated);border:1px solid var(--color-border);font-size:0.85rem;color:var(--color-text-primary);cursor:pointer;">
    ...
    const renderCurrentView = () => {
      ...
      if (localLibraryService.localTracks.length === 0) {
        ...
        contentArea.innerHTML = `
          <div style="text-align:center;padding:60px 20px;background:var(--color-bg-card);border:1px dashed var(--color-border);border-radius:18px;margin-top:10px;">
            <div style="width:72px;height:72px;margin:0 auto 16px;border-radius:20px;background:rgba(255,45,85,0.1);display:flex;align-items:center;justify-content:center;color:var(--color-accent);">
              <svg width="36" height="36" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M22 19a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h5l2 3h9a2 2 0 0 1 2 2z"></path><polyline points="12 11 12 17"></polyline><line x1="9" y1="14" x2="15" y2="14"></line></svg>
            </div>
            <h2 style="font-size:1.4rem;font-weight:700;margin-bottom:8px;">No Local Songs Added Yet</h2>
            <p style="color:var(--color-text-muted);font-size:0.9rem;max-width:440px;margin:0 auto 24px;line-height:1.5;">
              Import individual audio files or entire music folders from your device. All files remain strictly on your device with offline playback and tag parsing.
            </p>
            <div style="display:flex;align-items:center;justify-content:center;gap:12px;flex-wrap:wrap;">
              <button class="btn-primary" id="empty-add-files-btn" style="padding:10px 22px;">
                ${ICONS.plus} Select Music Files
              </button>
              <button class="btn-secondary" id="empty-add-folder-btn" style="padding:10px 22px;">
                Select Music Folder
              </button>
            </div>
          </div>
            ...
            ${Object.entries(artistMap).map(([artistName, songList]) => `
              <div class="music-card local-artist-card" style="cursor:pointer;" data-artist="${artistName}">
                <div class="music-card-cover-wrap">
                  <img src="${songList[0]?.artwork || "https://images.unsplash.com/photo-1511671782779-c97d3d27a1d4?w=500&q=80"}" style="border-radius:50%;" alt="${artistName}">
          ...
          <div class="content-grid" id="local-albums-grid">
            ${Object.entries(albumMap).map(([albumName, songList]) => `
              <div class="music-card local-album-card" style="cursor:pointer;" data-album="${albumName}">
                <div class="music-card-cover-wrap">
                  <img src="${songList[0]?.artwork || "https://images.unsplash.com/photo-1514525253161-7a46d19cd819?w=500&q=80"}" alt="${albumName}">
                  <div class="music-card-play-overlay">
                    <button class="play-pill-btn">${ICONS.play}</button>
                  </div>
                </div>
    ...
    fileInput?.addEventListener("change", async (e) => {
      if (e.target.files && e.target.files.length) {
        ...
        try {
          const imported = await localLibraryService.importFiles(e.target.files);
          showToast(`Successfully added ${imported.length} local song(s)!`, "success");
          this.renderLocalMusic();
        } catch (error) {
          console.error("Local file import failed:", error);
          showToast("Could not import the selected audio files.");
        }
        e.target.value = "";
      }
    });
    ...
    folderInput?.addEventListener("change", async (e) => {
      if (e.target.files && e.target.files.length) {
        ...
        try {
          ...
          showToast("Could not import the selected folder audio.");
        }
        e.target.value = "";
      }
    });
      ...
      <div>
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px;flex-wrap:wrap;gap:12px;">
          <div>
            ...
            <p style="color:var(--color-text-muted);font-size:0.85rem;">${state.songs.length} tracks in studio quality</p>
          </div>
          <div style="display:flex;align-items:center;gap:10px;">
        ...
        <div style="display:flex;align-items:center;gap:12px;margin-bottom:16px;">
          <input type="text" id="songs-filter-input" placeholder="Filter current list..." style="padding:6px 14px;border-radius:var(--radius-full);background:var(--color-bg-surface-elevated);border:1px solid var(--color-border);font-size:0.85rem;color:var(--color-text-primary);width:220px;">
          <select id="songs-sort-select" style="padding:6px 14px;border-radius:var(--radius-full);background:var(--color-bg-surface-elevated);border:1px solid var(--color-border);font-size:0.85rem;color:var(--color-text-primary);cursor:pointer;">
            <option value="title">Sort: Title</option>
            <option value="artist">Sort: Artist</option>
            <option value="album">Sort: Album</option>
            <option value="duration">Sort: Duration</option>
  ...
  renderRadio() {
    this.viewport.innerHTML = `
      <div>
        <h1 style="font-size:2rem;font-weight:800;margin-bottom:8px;">Live Internet Radio</h1>
        <p style="color:var(--color-text-muted);margin-bottom:24px;font-size:0.9rem;">High-fidelity continuous audio broadcasts across global frequencies</p>
        <div class="content-grid" id="radio-grid"></div>
      </div>
    `;
    const grid = document.getElementById("radio-grid");
    state.radioStations.forEach(st => {
  ...
  renderEqualizer() {
    ...
    const presetsHtml = Object.keys(EQ_PRESETS).map(name => `
      <button class="preset-btn ${state.eqPreset === name ? 'active' : ''}" data-preset="${name}">
        ${name}
      </button>
    `).join("");
    ...
    this.viewport.innerHTML = `
      <div class="eq-page-wrapper">
        ...
        <div class="eq-container">
          <div class="eq-header">
            <span style="font-size:0.85rem;font-weight:700;text-transform:uppercase;color:var(--color-text-muted);">10-Band Spectrum Adjustment</span>
            <div class="eq-presets-wrap" style="display:flex;gap:8px;flex-wrap:wrap;">${presetsHtml}</div>
          </div>
    ...
    this.viewport.innerHTML = `
      <div style="height:100%;display:flex;flex-direction:column;position:relative;">
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px;flex-wrap:wrap;gap:12px;">
          ...
          <div style="display:flex;gap:6px;background:var(--color-bg-card);padding:4px;border-radius:var(--radius-full);overflow-x:auto;max-width:100%;-webkit-overflow-scrolling:touch;">
            <button class="vis-mode-btn ${visualizerEngine.mode === 'bars' ? 'active' : ''}" data-mode="bars" style="padding:6px 12px;border-radius:var(--radius-full);font-size:0.8rem;font-weight:600;">Bars</button>
            <button class="vis-mode-btn ${visualizerEngine.mode === 'spectrum' ? 'active' : ''}" data-mode="spectrum" style="padding:6px 12px;border-radius:var(--radius-full);font-size:0.8rem;font-weight:600;">Waveform</button>
            <button class="vis-mode-btn ${visualizerEngine.mode === 'wave' ? 'active' : ''}" data-mode="wave" style="padding:6px 12px;border-radius:var(--radius-full);font-size:0.8rem;font-weight:600;">Oscilloscope</button>
            <button class="vis-mode-btn ${visualizerEngine.mode === 'circular' ? 'active' : ''}" data-mode="circular" style="padding:6px 12px;border-radius:var(--radius-full);font-size:0.8rem;font-weight:600;">Circular</button>
            <button class="vis-mode-btn ${visualizerEngine.mode === 'particles' ? 'active' : ''}" data-mode="particles" style="padding:6px 12px;border-radius:var(--radius-full);font-size:0.8rem;font-weight:600;">Particles</button>
          </div>
        </div>
  ...
  renderSettings() {
      ...
      <div class="settings-container">
        ...
        <div style="display:flex;flex-direction:column;gap:20px;">
          ...
          <div class="settings-card">
            <h3 style="font-size:1.1rem;font-weight:700;margin-bottom:14px;">Visual Appearance</h3>
            <div class="theme-select-grid">
              <button class="theme-select-btn ${state.settings.theme === 'dark' ? 'active' : ''}" data-theme-val="dark">
                <span class="theme-dot dark-dot"></span>
                <span>Dark Mode</span>
              </button>
              <button class="theme-select-btn ${state.settings.theme === 'light' ? 'active' : ''}" data-theme-val="light">
                <span class="theme-dot light-dot"></span>
                <span>Light Mode</span>
              </button>
              <button class="theme-select-btn ${state.settings.theme === 'amoled' ? 'active' : ''}" data-theme-val="amoled">
                <span class="theme-dot amoled-dot"></span>
            ...
            <div style="display:flex;gap:10px;flex-wrap:wrap;">
              <button class="accent-btn" data-accent="default" style="width:32px;height:32px;border-radius:50%;background:#ff2d55;border:2px solid #fff;"></button>
              <button class="accent-btn" data-accent="emerald" style="width:32px;height:32px;border-radius:50%;background:#10b981;border:2px solid transparent;"></button>
              <button class="accent-btn" data-accent="electric-blue" style="width:32px;height:32px;border-radius:50%;background:#2563eb;border:2px solid transparent;"></button>
              <button class="accent-btn" data-accent="sunset-amber" style="width:32px;height:32px;border-radius:50%;background:#f59e0b;border:2px solid transparent;"></button>
              <button class="accent-btn" data-accent="cyber-magenta" style="width:32px;height:32px;border-radius:50%;background:#d946ef;border:2px solid transparent;"></button>
            </div>
          </div>
          ...
          <div class="settings-card" style="display:flex;flex-direction:column;gap:16px;">
            ...
            <label style="display:flex;align-items:center;justify-content:space-between;cursor:pointer;">
              <div>
                <div style="font-weight:600;font-size:0.92rem;">Volume Normalization</div>
                <div style="font-size:0.75rem;color:var(--color-text-muted);">Dynamically level audio tracks to prevent sudden loudness shifts.</div>
              </div>
              <input type="checkbox" id="set-normalize" ${state.settings.normalizeVolume ? 'checked' : ''} style="width:20px;height:20px;accent-color:var(--color-accent);">
            </label>

            <label style="display:flex;align-items:center;justify-content:space-between;cursor:pointer;">
              <div>
                <div style="font-weight:600;font-size:0.92rem;">Reduce Motion</div>
                <div style="font-size:0.75rem;color:var(--color-text-muted);">Minimize animations, spins, and transitions across UI.</div>
              </div>
              <input type="checkbox" id="set-motion" ${state.settings.reduceMotion ? 'checked' : ''} style="width:20px;height:20px;accent-color:var(--color-accent);">
            </label>
          </div>
          ...
          <div class="settings-card" style="display:flex;flex-direction:column;gap:16px;">
            <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:10px;">
              <div>
                ...
                <p style="font-size:0.8rem;color:var(--color-text-muted);margin-top:2px;">Real-time clock, Open-Meteo weather intelligence, and weather-aware soundscapes.</p>
              </div>
            ...
            <div style="display:grid;grid-template-columns:repeat(auto-fit, minmax(220px, 1fr));gap:14px;padding-top:4px;">
              <div>
                ...
                <div style="display:flex;gap:8px;">
                  <button class="btn-secondary ${ambientContext.config.tempUnit === 'C' ? 'active' : ''}" id="settings-temp-c" style="flex:1;padding:8px;font-weight:700;${ambientContext.config.tempUnit === 'C' ? 'background:var(--color-accent);color:#fff;border-color:var(--color-accent);' : ''}">Celsius (°C)</button>
                  <button class="btn-secondary ${ambientContext.config.tempUnit === 'F' ? 'active' : ''}" id="settings-temp-f" style="flex:1;padding:8px;font-weight:700;${ambientContext.config.tempUnit === 'F' ? 'background:var(--color-accent);color:#fff;border-color:var(--color-accent);' : ''}">Fahrenheit (°F)</button>
                </div>
              </div>
              ...
              <div>
                ...
                <div style="display:flex;gap:8px;">
                  <button class="btn-secondary ${ambientContext.config.timeFormat === '12h' ? 'active' : ''}" id="settings-time-12" style="flex:1;padding:8px;font-weight:700;${ambientContext.config.timeFormat === '12h' ? 'background:var(--color-accent);color:#fff;border-color:var(--color-accent);' : ''}">12-Hour (AM/PM)</button>
                  <button class="btn-secondary ${ambientContext.config.timeFormat === '24h' ? 'active' : ''}" id="settings-time-24" style="flex:1;padding:8px;font-weight:700;${ambientContext.config.timeFormat === '24h' ? 'background:var(--color-accent);color:#fff;border-color:var(--color-accent);' : ''}">24-Hour</button>
                </div>
              </div>
            </div>
          ...
          <div class="settings-card" style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:14px;">
    ...
    this.viewport.querySelectorAll(".theme-select-btn").forEach(btn => {
      btn.addEventListener("click", () => {
        state.updateSettings({ theme: btn.dataset.themeVal });
        this.renderSettings();
      });
    });
